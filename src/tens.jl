...
struct Tens{order,dim,T,A<:AbstractArray} <: AbstractTens{order,dim,T}
    ...
    function Tens(
        array::AbstractArray{T,order},
        basis::Basis{dim},
        var::NTuple{order,Symbol} = ntuple(_ -> :cont, Val(order)),
    ) where {order,dim,T}
        newarray = tensor_or_array(array)
        new{order,dim,T,typeof(newarray)}(newarray, basis, var)
    end
    Tens(array::AbstractArray, basis::RotatedBasis, args...) = TensRotated(array, basis)
    Tens(array::AbstractArray, basis::OrthogonalBasis, args...) =
        TensOrthogonal(array, basis, args...)
    Tens(array::AbstractArray, ::CanonicalBasis, args...) = TensCanonical(array)
    Tens(array::AbstractArray, args...) = TensCanonical(array)
    Tens(array::AbstractArray, var::NTuple, basis::AbstractBasis) = Tens(array, basis, var)
end
...
struct TensRotated{order,dim,T,A<:AbstractArray} <: AbstractTens{order,dim,T}
    ...
    function TensRotated(
        ...
        newarray = tensor_or_array(array)
        new{order,dim,T,typeof(newarray)}(newarray, basis)
    end
end

struct TensCanonical{order,dim,T,A<:AbstractArray} <: AbstractTens{order,dim,T}
    array::A
    function TensCanonical(array::AbstractArray{T,order}) where {order,T}
        newarray = tensor_or_array(array)
        new{order,size(array,1),T,typeof(newarray)}(newarray)
    end
end
...
@pure arraytype(::TensArray{order,dim,T,A}) where {order,dim,T,A} = A
...
function components(
    ...
    if var == getvar(t)
        ...
        for i ∈ 1:order
            if getvar(t, i) ≠ var[i]
                g_or_G = metric(ℬ, var[i])
                for d ∈ 1:dim
                    ec = ntuple(j -> j ≠ i ? Colon() : d, Val(order))
                    m[ec...] *= g_or_G[d, d]
                end
            end
        end
...
for OP in (:(simplify), :(factor), :(subs))
    @eval SymPy.$OP(
        ...
        args...;
        kwargs...,
    ) where {order,dim} = Tens($OP(getarray(t); kwargs...), getbasis(t), getvar(t))
...
# SymPy.simplify(t::AbstractTens{order,dim,Sym}; kwargs...) where {order,dim} = Tens(simplify(array(t); kwargs...), basis(t), getvar(t))
...
# sympy.trigsimp(t::AbstractTens{order,dim,Sym}; kwargs...) where {order,dim} = Tens(trigsimp(array(t); kwargs...), basis(t), getvar(t))
...
# SymPy.factor(t::AbstractTens{order,dim,Sym}; kwargs...) where {order,dim} = Tens(factor(array(t); kwargs...), basis(t), getvar(t))
...
# SymPy.subs(t::AbstractTens{order,dim,Sym}, d...; kwargs...) where {order,dim} = Tens(subs(array(t), d...; kwargs...), basis(t), getvar(t))
...
for OP in (:+, :-)
    @eval function Base.$OP(
        ...
        nt1, nt2 = same_basis_same_getvar(t1, t2)
        return Tens($OP(getarray(nt1), getarray(nt2)), getvar(nt1), getbasis(nt1))
    end
end
